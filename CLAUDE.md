...
- Ingredient tables use two columns both headed `Ingredienti`, with each cell formatted as `**quantity** - ingredient name`, placing two ingredients per row
...
- When a YouTube URL is provided for a recipe, extract the video title and description. Do not fetch the page directly but use Playwright MCP to extract the data. If this fails or there is no description, ask the user to paste the title and description text directly. Once the description is available, extract ingredients and steps from it.
